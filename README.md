...
特に回転時に意図しない軸へ動きが逃げないよう、**各パーツの動作方向を基準に回転軸を設定**しています。

これにより、Controllerを感覚的に操作可能にし、すべてのコントローラーで同じ値で回転するようになっています。アニメーション時の操作性を担保しています。
...
という要望があったため、背面パーツにも個別に操作できるリグを追加しました。

シルエットの変化や攻撃モーションなど、アニメーション側で必要になる表現をリグ制作時点で固定せず、**アニメーターが必要に応じて調整できる構造**にしています。
